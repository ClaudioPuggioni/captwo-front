...
export default function BookingsDoctor() {
  ...
  const available_times_Parser = function (inputObj) {
    ...
    const currWeekdaySLUG3 = currentDay.slice(0, 3).toLocaleLowerCase();
  ...
  const handleSubmit = () => {
    dispatch(registerConsultation({ doctorId: chosenDoctor.id, patientId: userInfo.id, consult_reason: "", date: bookedSlot }));
  };
  ...
  return (
    <div id="bookingsContainer" style={{ position: "relative" }}>
      <div style={{ position: "absolute", top: 0, bottom: 0, minHeight: "100%", width: "100%" }}>
              ...
              ? Object.entries(timetable[0][weekdayOrder.find((ele) => ele.slice(0, 3) === value["$d"].toString().slice(0, 3).toLowerCase())]).map(
                  (button, idx) => (
                    <button
                      ...
                      style={{
                        width: "140px",
                        backgroundColor: button[1] && button[0] !== bookedSlot[1] ? "#6dce6d" : button[0] === bookedSlot[1] ? "#1c58b1" : "#f4d5da",
                        padding: "7px 15px",
        ...
        <HealMeButtonCustom onClick={handleSubmit} disabled={loading || userType === "Doctor"}>
